% \subsection{Overview} - I feel I don't need a header here, this is just the opening text of the method that pre-empts the breakdown.
...
From metadata, each directed edge $(j \rightarrow i)$ denotes that paper $p_i$ cites paper $p_j$. We resolve any cylic instances by removing edges where publication year$(p_j)$ exceeds publication year$(p_i)$. % better word for "cylic instances" - cylic problems or something idk

For each focal paper $p_i$, define its parent set $\mathcal{P}(i)$ as cited predecessors retained after filtering. Papers with fewer than $k_{\min}$ valid parents are kept in the corpus for descriptive analysis but excluded from inheritance regression to avoid unstable estimates. % why the last statement? Why would they be unstable? Why couldn't they be used. Do we need to even note k_min ? 
